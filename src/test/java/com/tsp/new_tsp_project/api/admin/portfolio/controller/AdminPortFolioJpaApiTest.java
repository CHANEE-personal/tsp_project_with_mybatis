...
class AdminPortFolioJpaApiTest {
    ...
    public void 포트폴리오조회() throws Exception {
        mockMvc.perform(get("/api/jpa-portfolio/lists").param("page", "1").param("size", "100"))
                .andDo(print())
    ...
    public void 포트폴리오상세조회() throws Exception {
        mockMvc.perform(get("/api/jpa-portfolio/1"))
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.portFolioInfo.title").value("포트폴리오 테스트"))
                .andExpect(jsonPath("$.portFolioInfo.description").value("포트폴리오 테스트"))
                .andExpect(jsonPath("$.portFolioInfo.hashTag").value("TEST"))
                .andExpect(jsonPath("$.portFolioInfo.videoUrl").value("http://youtube.com"));
